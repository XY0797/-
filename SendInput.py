...
class MouseInput(ctypes.Structure):
    _fields_ = [
        (
            "dx",
            ctypes.c_long,
        ),  # 水平方向的绝对位置/相对移动量(像素), dwFlags 中包含 MOUSEEVENTF_ABSOLUTE 标识就是绝对移动, 否则是相对移动
        ...
        (
            "dwFlags",
            ctypes.c_ulong,
        ),  # 事件标识集, 可以是移动或点击事件的合理组合, 即可以一个命令实现移动且点击
        (
            "time",
            ctypes.c_ulong,
        ),  # 事件发生的时间戳, 可以指定发生的时间? 传入0则使用系统提供的时间戳
        ("dwExtraInfo", ctypes.POINTER(ctypes.c_ulong)),
    ]  # 应用可通过 GetMessageExtraInfo 来接收通过此参数传递的额外消息
...
class KeyboardInput(ctypes.Structure):
    _fields_ = [
        ...
        (
            "wScan",
            ctypes.c_ushort,
        ),  # 键的硬件扫描码, 如果dwFlags指定了KEYEVENTF_UNICODE, 则wScan需为一个Unicode字符
...
class Inner(
    ctypes.Union
):  # 共用体, 和结构体类似, 但是各成员属性共用同一块内存空间, 实例化时的空间大小就是成员属性中最大的那个的空间大小, 实例化时只能赋值ki/mi/hi中的一个
    _fields_ = [("ki", KeyboardInput), ("mi", MouseInput), ("hi", HardwareInput)]


class Input(ctypes.Structure):
    _fields_ = [("type", ctypes.c_ulong), ("ii", Inner)]  # 输入事件类型
...
MOUSEEVENTF_WHEEL = (
    0x0800  # 滚轮垂直滚动, mouseData需传入滚动值, 一个滚动单位是120(像素?)
)
...
XBUTTON1 = (
    0x0001  # 侧下键, dwFlags为MOUSEEVENTF_XDOWN/MOUSEEVENTF_XUP时, mouseData需传入x键值
)
XBUTTON2 = 0x0002  # 侧上键
...
KEYEVENTF_SCANCODE = (
    0x0008  # 指定该标记, 就是使用硬件扫描码来发送按键按下事件, wVk参数将被忽略
)
...
class Keyboard:
    ...
    def press(
        wVk,
    ):  # 十六进制虚拟键码, 范围在[1,254], 如果dwFlags指定了KEYEVENTF_UNICODE, 则wVk必须是0, https://learn.microsoft.com/en-us/windows/win32/inputdev/virtual-key-codes
        return SendInput(
            Input(INPUT_KEYBOARD, Inner(ki=KeyboardInput(wVk, 0, 0, 0, None)))
        )
    ...
    def release(wVk):
        return SendInput(
            Input(
                INPUT_KEYBOARD,
                Inner(ki=KeyboardInput(wVk, 0, KEYEVENTF_KEYUP, 0, None)),
            )
        )
    ...
    def pressByScanCode(
        wScan,
    ):  # 十六进制键扫描码, 可通过该工具获得, https://github.com/Lateralus138/Key-ScanCode/releases/tag/1.9.30.18
        return SendInput(
            Input(
                INPUT_KEYBOARD,
                Inner(ki=KeyboardInput(0, wScan, KEYEVENTF_SCANCODE, 0, None)),
            )
        )
    ...
    def releaseByScanCode(wScan):
        return SendInput(
            Input(
                ...
                Inner(
                    ki=KeyboardInput(
                        0, wScan, KEYEVENTF_SCANCODE | KEYEVENTF_KEYUP, 0, None
                    )
                ),
            )
        )
    ...
    @staticmethod
    def pressByUnicode(charStr):
        str_byte = charStr.encode("utf-16-le")
        parm_list = []
        for i in range(0, len(str_byte), 2):
            # 每两个字节转int16
            wScan = (str_byte[i + 1] << 8) + str_byte[i]
            # 十六进制Unicode编码, 比如 汉字 "一" 的十六进制UTF8编码为 0x4e00, 则可通过发送该编码直接输入汉字一
            parm_list.append(
                Input(
                    INPUT_KEYBOARD,
                    Inner(ki=KeyboardInput(0, wScan, KEYEVENTF_UNICODE, 0, None)),
                )
            )
        # parm_list转tuple
        return SendInput(*tuple(parm_list))

    @staticmethod
    def releaseByUnicode(charStr):
        str_byte = charStr.encode("utf-16-le")
        parm_list = []
        for i in range(0, len(str_byte), 2):
            # 每两个字节转int16
            ...
            parm_list.append(
                Input(
                    ...
                    Inner(
                        ki=KeyboardInput(
                            0, wScan, KEYEVENTF_UNICODE | KEYEVENTF_KEYUP, 0, None
                        )
                    ),
                )
            )
...
class Mouse:
    ...
    def leftDown():
        return SendInput(
            Input(
                INPUT_MOUSE,
                Inner(mi=MouseInput(0, 0, 0, MOUSEEVENTF_LEFTDOWN, 0, None)),
            )
        )
    ...
    def leftUp():
        return SendInput(
            Input(
                INPUT_MOUSE, Inner(mi=MouseInput(0, 0, 0, MOUSEEVENTF_LEFTUP, 0, None))
            )
        )
    ...
    def leftClick():
        return SendInput(
            Input(
                ...
                Inner(
                    mi=MouseInput(
                        0, 0, 0, MOUSEEVENTF_LEFTDOWN | MOUSEEVENTF_LEFTUP, 0, None
                    )
                ),
            )
        )
    ...
    def rightDown():
        return SendInput(
            Input(
                INPUT_MOUSE,
                Inner(mi=MouseInput(0, 0, 0, MOUSEEVENTF_RIGHTDOWN, 0, None)),
            )
        )
    ...
    def rightUp():
        return SendInput(
            Input(
                INPUT_MOUSE, Inner(mi=MouseInput(0, 0, 0, MOUSEEVENTF_RIGHTUP, 0, None))
            )
        )
    ...
    def rightClick():
        return SendInput(
            Input(
                ...
                Inner(
                    mi=MouseInput(
                        0, 0, 0, MOUSEEVENTF_RIGHTDOWN | MOUSEEVENTF_RIGHTUP, 0, None
                    )
                ),
            )
        )
    ...
    def middleDown():
        return SendInput(
            Input(
                INPUT_MOUSE,
                Inner(mi=MouseInput(0, 0, 0, MOUSEEVENTF_MIDDLEDOWN, 0, None)),
            )
        )
    ...
    def middleUp():
        return SendInput(
            Input(
                INPUT_MOUSE,
                Inner(mi=MouseInput(0, 0, 0, MOUSEEVENTF_MIDDLEUP, 0, None)),
            )
        )
    ...
    def middleClick():
        return SendInput(
            Input(
                ...
                Inner(
                    mi=MouseInput(
                        0, 0, 0, MOUSEEVENTF_MIDDLEDOWN | MOUSEEVENTF_MIDDLEUP, 0, None
                    )
                ),
            )
        )
    ...
    def x1Down():  # 侧下键
        return SendInput(
            Input(
                INPUT_MOUSE,
                Inner(mi=MouseInput(0, 0, XBUTTON1, MOUSEEVENTF_XDOWN, 0, None)),
            )
        )
    ...
    def x1Up():
        return SendInput(
            Input(
                INPUT_MOUSE,
                Inner(mi=MouseInput(0, 0, XBUTTON1, MOUSEEVENTF_XUP, 0, None)),
            )
        )
    ...
    def x1Click():
        return SendInput(
            Input(
                ...
                Inner(
                    mi=MouseInput(
                        0, 0, XBUTTON1, MOUSEEVENTF_XDOWN | MOUSEEVENTF_XUP, 0, None
                    )
                ),
            )
        )
    ...
    def x2Down():  # 侧上键
        return SendInput(
            Input(
                INPUT_MOUSE,
                Inner(mi=MouseInput(0, 0, XBUTTON2, MOUSEEVENTF_XDOWN, 0, None)),
            )
        )
    ...
    def x2Up():
        return SendInput(
            Input(
                INPUT_MOUSE,
                Inner(mi=MouseInput(0, 0, XBUTTON2, MOUSEEVENTF_XUP, 0, None)),
            )
        )
    ...
    def x2Click():
        return SendInput(
            Input(
                ...
                Inner(
                    mi=MouseInput(
                        0, 0, XBUTTON2, MOUSEEVENTF_XDOWN | MOUSEEVENTF_XUP, 0, None
                    )
                ),
            )
        )
    ...
    def move(x, y, absolute=False):
        if not absolute:
            return SendInput(
                Input(
                    INPUT_MOUSE,
                    Inner(mi=MouseInput(x, y, 0, MOUSEEVENTF_MOVE, 0, None)),
                )
            )
        else:
            # 绝对值移动时, 屏幕宽高的取值范围都是[0, 65535], 需要自行换算一下
            # 获取显示分辨率(非物理分辨率)
            w, h = ctypes.windll.user32.GetSystemMetrics(
                0
            ...
            return SendInput(
                Input(
                    ...
                    Inner(
                        mi=MouseInput(
                            rx, ry, 0, MOUSEEVENTF_MOVE | MOUSEEVENTF_ABSOLUTE, 0, None
                        )
                    ),
                )
            )
    ...
    def scroll(
        delta, vertical=True
    ):  # 滚动delta个单位, 一个单位是120(像素?), 区分正负值, 正值:向下转/向右转, 负值:向上移/向左转
        dwFlags = MOUSEEVENTF_WHEEL if vertical else MOUSEEVENTF_HWHEEL
        return SendInput(
            Input(
                INPUT_MOUSE,
                Inner(mi=MouseInput(0, 0, delta * WHEEL_DELTA, dwFlags, 0, None)),
            )
        )
...
if __name__ == "__main__":
    ...
    print(Keyboard.pressByUnicode("一"))
